...
export default function PlanetList({ onPlanetClick, onSunClick, activePlanet }: PlanetListProps) {
  return (
    <div className="fixed left-0 top-0 bottom-0 w-16 md:w-[72px] glass-strong z-30 flex flex-col items-center py-4 gap-2.5 overflow-y-auto">
      {/* Sun */}
      <button
        onClick={onSunClick}
        className="tooltip-planet group relative flex items-center justify-center w-11 h-11 md:w-12 md:h-12 rounded-full transition-all hover:scale-110 cursor-pointer mb-1 flex-shrink-0"
        data-tooltip="☀️ Sol"
        style={{
      ...
      {planets.map((planet) => {
        ...
        return (
          <button
            ...
            className="tooltip-planet group relative flex items-center justify-center rounded-full transition-all hover:scale-115 cursor-pointer flex-shrink-0"
