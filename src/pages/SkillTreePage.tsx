...
const difficultyColor = (d: string) => {
  if (d === "básico") return "text-[#72EB3A]";
            ...
            <div className="grid grid-cols-2 gap-3">
              {enrolledSubjects.map((sub) => (
                <button
                  ...
                  className="bg-[#1C2210] border border-[#365A08] rounded-2xl p-4 text-left hover:border-[#72EB3A]/50 transition-colors group active:scale-95"
                >
                  <div className="text-3xl mb-2">{sub.emoji}</div>
                  <p className="font-bold text-sm text-white group-hover:text-[#72EB3A] transition-colors">{sub.name}</p>
    ...
    return (
      <div className="min-h-screen bg-[#1B1D24] text-white flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-16 h-16 border-4 border-[#72EB3A] border-t-transparent rounded-full animate-spin" />
      ...
      <div className="max-w-md mx-auto w-full px-5 py-6">
        ...
        <div className="flex items-center gap-3 mb-6 mt-2">
          ...
          <div className="flex-1">
            <p className="text-[#72EB3A] text-[10px] font-black tracking-widest uppercase">Árvore de Habilidades</p>
            <h1 className="text-2xl font-bold">{subject}</h1>
          </div>
          <button
            onClick={() => { setSubject(""); setTopics([]); }}
            className="text-xs text-[#72EB3A] bg-[#72EB3A]/10 px-3 py-1.5 rounded-xl font-bold"
          >
            Trocar
          </button>
        </div>
        ...
        <div className="bg-[#1C2210] border border-[#365A08] rounded-2xl p-4 mb-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-[#72EB3A] font-black text-sm">{totalCrowns}/{maxCrowns} 👑</span>
          </div>
          <div className="h-2.5 bg-[#1B1D24] rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-[#72EB3A] to-[#5D9D0B] rounded-full transition-all duration-700"
          ...
          {topics.map((topic, idx) => {
            ...
            return (
              <div key={topic.id} className="relative">
                ...
                {idx > 0 && (
                  <div
                    className={`absolute w-0.5 h-8 left-1/2 -translate-x-1/2 -top-8 ${c > 0 ? "bg-[#72EB3A]/50" : "bg-slate-800"}`}
                  />
                )}
                ...
                >
                  ...
                  >
                    ...
                    {status === "available" && c === 0 && (
                      <div className="absolute inset-0 rounded-full bg-[#72EB3A]/20 animate-pulse scale-150" />
                    )}
                    ...
                    <div
                      className={`w-[72px] h-[72px] rounded-full flex flex-col items-center justify-center border-4 relative z-10 shadow-lg transition-all
                        ${isMastered ? "border-yellow-400 bg-gradient-to-br from-yellow-500/20 to-yellow-900/20 shadow-yellow-500/20" :
                          c > 0 ? "border-[#72EB3A] bg-[#253510] shadow-[#72EB3A]/20" :
                          status === "available" ? "border-[#72EB3A]/60 bg-[#1C2210] group-hover:border-[#72EB3A]" :
                          "border-slate-700 bg-[#1C2210]"
                        }`}
                    >
                      {status === "locked"
                  ...
                  <div className={`flex-1 ${isLeft ? "text-left" : "text-right"}`}>
                    <p className={`text-sm font-bold leading-tight ${isMastered ? "text-yellow-400" : c > 0 ? "text-[#72EB3A]" : "text-white"}`}>
                      {topic.name}
                    </p>
                    {isMastered && (
                    ...
                    {c > 0 && !isMastered && (
                      <span className="text-[10px] text-[#72EB3A]/70 font-bold">{c}/5 coroas</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col items-center mt-4 mb-8">
            ...
            <div className={`w-20 h-20 rounded-full border-4 flex flex-col items-center justify-center
              ${masteredCount === topics.length
                ...
                : "border-slate-700 bg-[#1C2210] opacity-50"
              }`}
            >
