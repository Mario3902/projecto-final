...
const Tasks = () => {
  ...
  return (
    <div className="min-h-screen bg-[#1B1D24] text-white flex flex-col font-sans pb-24 relative overflow-x-hidden">
      <div className="max-w-md mx-auto w-full px-5 py-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between mt-2 mb-8">
          <div className="flex items-center gap-3">
            <div className="h-11 w-11 rounded-full bg-[#253510] border-2 border-[#72EB3A]/30 shadow-[0_0_15px_rgba(74,222,128,0.15)] flex items-center justify-center overflow-hidden">
              <span className="text-2xl mt-1">👨🏻‍💼</span>
            </div>
            <div>
              <h3 className="text-[#72EB3A] text-[10px] font-black tracking-widest uppercase mb-0.5">NÍVEL {level}</h3>
              <div className="h-1.5 w-24 bg-[#253510] rounded-full overflow-hidden">
                <div className="h-full bg-[#72EB3A]" style={{ width: "40%" }}></div>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 bg-[#1C2210] border border-slate-800 px-3 py-1.5 rounded-full">
              <Flame className="h-3.5 w-3.5 fill-[#72EB3A] text-[#72EB3A]" />
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-3 text-slate-400">
              <button className="h-8 w-8 rounded-full bg-[#1C2210] border border-slate-800 flex items-center justify-center hover:text-[#72EB3A] transition-colors"><ChevronLeft className="h-4 w-4" /></button>
              <button className="h-8 w-8 rounded-full bg-[#1C2210] border border-slate-800 flex items-center justify-center hover:text-[#72EB3A] transition-colors"><ChevronRight className="h-4 w-4" /></button>
            </div>
          </div>
          
          <div className="bg-[#1C2210] rounded-2xl p-4 flex justify-between items-center border border-slate-800/60 shadow-lg">
            {currentWeek.map((d, i) => (
              <div 
                key={i} 
                className={`flex flex-col items-center justify-center rounded-xl py-2 px-1 w-[42px] relative transition-all duration-300 ${
                  d.isToday ? "bg-[#72EB3A] text-[#1B1D24] shadow-[0_4px_15px_rgba(74,222,128,0.2)]" : "text-slate-400"
                }`}
              >
                <span className="text-[9px] font-bold mb-1 uppercase">{d.dayStr}</span>
                <span className={`text-[15px] font-black ${d.isToday ? "text-[#1B1D24]" : "text-slate-300"}`}>{d.num}</span>
                
                {/* Indica se estudou no dia */}
                {d.hasStudied && !d.isToday && (
                  <div className="absolute -bottom-1 h-1.5 w-1.5 bg-[#72EB3A] rounded-full shadow-[0_0_5px_#72EB3A]"></div>
                )}
                {d.hasStudied && d.isToday && (
                  <div className="absolute -bottom-1 h-1.5 w-1.5 bg-[#1B1D24] rounded-full"></div>
                )}
              </div>
            ))}
          </div>
        ...
        {/* POMODORO SYSTEM */}
        <h2 className="text-[19px] font-bold mb-4">Sessão Pomodoro <span className="text-[#72EB3A] text-sm ml-2">🍅 Recompensa: 25 XP</span></h2>
        
        {isConfiguring && !isBreak ? (
           // SETUP POMODORO
           <div className="bg-[#1C2210] border border-slate-800/60 rounded-3xl p-5 mb-8 shadow-lg">
             ...
             {subjects.length > 0 ? (
               <div className="flex gap-2 overflow-x-auto pb-3 mb-2 scrollbar-hide">
                 {subjects.map(s => (
                   <button 
                     ...
                     className={`shrink-0 px-4 py-2.5 rounded-xl text-sm font-bold border transition-colors flex items-center gap-2 ${
                       selectedSubject?.id === s.id ? "bg-[#72EB3A]/10 border-[#72EB3A] text-[#72EB3A]" : "bg-[#1B1D24] border-slate-800 text-slate-400"
                     }`}
                   >
                ...
                <div className="text-xs text-slate-500 bg-[#1B1D24] p-3 rounded-xl border border-slate-800 mb-4">
             ...
             <input
                ...
                className="w-full bg-[#1B1D24] border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:border-[#72EB3A] outline-none transition-colors mb-3"
             />

             {selectedSubject && (
               <div className="mb-4">
                 <p className="text-[10px] font-bold text-[#72EB3A] uppercase tracking-widest mb-2 flex items-center gap-1.5 pt-1">
                 ...
                 <div className="flex flex-wrap gap-2">
                   {getSuggestionsForSubject(selectedSubject.name).map((sug, i) => (
                     <button
                       ...
                       className="bg-[#253510] hover:bg-[#365A08] border border-slate-700 text-slate-300 text-[11px] font-bold px-3 py-1.5 rounded-lg transition-colors text-left"
                     >
                       {sug}
                     </button>
                   ))}
             ...
             <button 
               ...
               className="w-full py-3.5 bg-[#72EB3A] text-[#1B1D24] font-black rounded-xl active:scale-95 transition-transform mt-2 flex items-center justify-center gap-2"
             >
           ...
           <div className={`border rounded-3xl p-6 mb-8 relative overflow-hidden flex justify-between items-center group transition-colors shadow-xl ${
             isBreak ? "bg-[#1e293b] border-blue-500/30" : isActive ? "bg-[#1C2210] border-[#72EB3A]/40" : "bg-[#1C2210] border-slate-800"
           }`}>
             
             {isActive && !isBreak && (
                <div className="absolute top-0 left-0 h-1 bg-[#72EB3A] transition-all" style={{ width: `${((25*60 - timeLeft) / (25*60)) * 100}%` }}></div>
             )}
             {isBreak && (
             ...
             <div className="relative z-10 w-full">
               <div className="flex justify-between items-start mb-2">
                 <div>
                    <h3 className={`text-[10px] font-black tracking-widest uppercase mb-1 ${isBreak ? "text-blue-400" : "text-[#72EB3A]"}`}>
                 ...
                 <button 
                  ...
                  className={`h-14 w-14 rounded-full flex items-center justify-center relative z-10 transition-transform active:scale-95 ${
                    isActive ? "bg-slate-800/80 hover:bg-slate-700" : isBreak ? "bg-blue-500" : "bg-[#72EB3A] shadow-[0_0_20px_rgba(74,222,128,0.3)]"
                  }`}
                 >
                  {isActive ? <Pause className="h-6 w-6 text-white fill-white" /> : <Play className="h-6 w-6 text-[#1B1D24] fill-current ml-1" />}
                 </button>
               </div>
               
               {!isBreak && (
                 <div className="flex items-center justify-between">
                   <p className="text-slate-400 text-xs font-bold bg-[#1B1D24] border border-slate-800 px-3 py-1.5 rounded-lg flex items-center gap-1.5 overflow-hidden text-ellipsis whitespace-nowrap max-w-[70%]">
        ...
        <div className="mb-8">
          <div className="flex justify-between items-end mb-4">
            ...
            <span className="text-xs text-slate-500 font-bold bg-[#1C2210] px-2.5 py-1.5 rounded-lg border border-slate-800">
          ...
          <div className="space-y-3">
             ...
             {pomodoroSessions.filter(s => new Date(s.completed_at || s.date).toDateString() === today.toDateString()).map(session => (
               <div key={`pom_${session.id}`} className="bg-[#1C2210]/80 border border-slate-800/80 rounded-2xl p-4 flex items-center gap-4">
                 <div className="h-10 w-10 rounded-xl bg-[#72EB3A]/10 flex items-center justify-center shrink-0">
                    <Trophy className="h-5 w-5 text-[#72EB3A]" />
                 </div>
                 ...
                 <div className="flex-1">
                   ...
                   <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                     {session.time || "CONCLUÍDO"} • <span className="text-[#72EB3A]">+ {session.xp_earned} XP</span>
                   </p>
                 </div>
               </div>
             ))}
      ...
      <button className="fixed bottom-24 right-5 h-14 w-14 rounded-2xl bg-[#1C2210] border border-[#365A08] shadow-lg flex items-center justify-center transition-transform active:scale-90 z-40 hover:border-[#72EB3A]">
        <Plus className="h-6 w-6 text-[#72EB3A]" />
      </button>
