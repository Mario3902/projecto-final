  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {nebulas.map((n, i) => (
        <div
          key={`nebula-${i}`}
          className="solar-nebula absolute rounded-full"
          style={{
            left: `${n.x}%`,
            top: `${n.y}%`,
            width: `${n.w}px`,
      ...
      {stars.map((star) => (
        <div
          key={star.id}
          className={`absolute rounded-full ${star.isBright ? "solar-animate-twinkle" : "animate-pulse"}`}
          style={{
            left: `${star.x}%`,
            top: `${star.y}%`,
            width: `${star.size}px`,
